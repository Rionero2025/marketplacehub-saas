...
export function OrderAccountPanel({ sellerId, account, environment, onBusyChange }: { sellerId: string; account: MarketplaceConnection; environment: OrderEnvironment; onBusyChange?: (busy: boolean) => void }) {
  ...
  useEffect(() => { onBusyChange?.(selectionPending || exporting !== null || trackingBusy); return () => onBusyChange?.(false); }, [onBusyChange, selectionPending, exporting, trackingBusy]);
  ...
  const actionBusy = loading || selectionPending || exporting !== null || trackingBusy;
  return <>
    ...
    {account.marketplace === "kaufland" && <OrderTrackingPanel sellerId={sellerId} accountId={account.id} environment={environment} orders={list?.items ?? []} disabled={loading || syncing || running(job) || selectionPending || exporting !== null} onBusyChange={setTrackingBusy} onMutated={async () => { setDetailVersion((value) => value + 1); await loadList(); }} onResponse={responseAllowed} />}
