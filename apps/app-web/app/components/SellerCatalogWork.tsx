...
export function SellerCatalogWork({sellerId}: {sellerId:string}) {
  ...
  async function match(){
    if(!saved||!recipe||!selectedList)return;
    const full=fullLists.find(f=>f.id===recipe.content_list_id)??(fullLists.length===1?fullLists[0]:undefined);
    if(!full)throw new Error("Seleziona un listino FULL pronto dello stesso fornitore.");
    const next=readWorkData(await request(`/work/views/${saved.id}/enrich`,"POST",{expected_revision:saved.revision,recipe:{...recipe,version:selectedList.active_version_number,content_list_id:full.id,content_version:full.active_version_number}}),sellerId,saved.id);
    if(!next)throw new Error("Verifica la vista prima di ripetere l’abbinamento.");
    setData(next);if(next.recipe)setRecipe(next.recipe);await loadIndex();
    const report=next.match_report;
    setSuccess(report?`Match completato su ${report.total} righe: FULL ${report.full_matched} abbinati, ${report.full_missing} mancanti, ${report.full_duplicate} duplicati. Stock LIGHT: ${report.light_matched} verificati, ${report.light_missing} mancanti, ${report.light_duplicate} duplicati.`:"Abbinamento completato.");
  }
  ...
  return <div className="catalog-work">
    ...
    {saved&&selectedList?.provider==="innpro"&&<section className="workspace-section workspace-section-body"><h2>Completa la vista con LIGHT + FULL</h2><p>Match per EAN esatto. Aggiorna quantità e stock dal LIGHT importato; completa nome, misure e informazioni mancanti dal FULL. Nomi, misure e prezzi già inseriti restano conservati. Per uno stock recente aggiorna prima il feed LIGHT.</p><label>FULL da abbinare<select value={recipe?.content_list_id??(fullLists.length===1?fullLists[0].id:"")} disabled={!writable} onChange={e=>setRecipe(recipe?{...recipe,content_list_id:e.target.value||null}:null)}><option value="">Scegli il FULL</option>{fullLists.map(f=><option key={f.id} value={f.id}>{f.name} · versione {f.active_version_number}</option>)}</select></label><button type="button" className="settings-save" disabled={!writable||!fullLists.length||Object.keys(edits).length>0||added.length>0||selection.length>0} onClick={()=>void run(match,true)}>Abbina EAN e completa dati</button><p className="workspace-field-note">Salva prima eventuali modifiche alla tabella. EAN duplicati non vengono abbinati. Quantità non verificabili restano segnalate e impediscono la pubblicazione automatica.</p></section>}
    ...
    {data && <form onSubmit={e=>{e.preventDefault();void run(save,true);}}>
      <section className="workspace-section"><div className="workspace-section-heading"><h2>{saved ? `Modifica vista · ${saved.name}` : "Prodotti nella vista"}</h2><span>{number.format(data.total)} prodotti · {number.format(selectedCount)} {saved?"mantenuti":"selezionati"}</span></div>
        ...
        <div className="catalog-table-wrap" tabIndex={0} role="region" aria-label="Prodotti della vista, scorrimento orizzontale"><table className="catalog-table catalog-work-table"><thead><tr><th>{saved?"Mantieni":"Seleziona"}</th><th>Nome prodotto</th><th>EAN</th><th>SKU</th><th>Peso (kg)</th><th>Dimensioni (cm)</th><th>Dati FULL / Match EAN</th>{economicFields.map(f=><th key={f}>{labels[f]}</th>)}</tr></thead><tbody>{[...data.rows,...added].map(row=><tr key={row.id}><td><input type="checkbox" aria-label={`${saved?"Mantieni":"Seleziona"} ${row.sku || row.ean}`} checked={all?!selection.includes(row.id):selection.includes(row.id)} disabled={pending} onChange={()=>toggle(row.id)} /></td>{(["name","ean","sku"] as const).map(f=><td key={f}>{saved?<input aria-label={`${f} ${row.sku || row.ean}`} value={edits[row.id]?.[f] ?? row[f]} disabled={!writable} maxLength={f==="ean"?128:f==="sku"?500:10000} onChange={e=>edit(row,f,e.target.value)} />:row[f] || "—"}</td>)}<td>{saved?<input aria-label={`Peso kg ${row.sku || row.ean}`} type="number" min="0" max="999999" step="any" value={edits[row.id]?.weight_kg ?? row.weight_kg ?? "0"} disabled={!writable} onChange={e=>edit(row,"weight_kg",e.target.value)} />:row.weight_kg===null?"—":number.format(Number(row.weight_kg))}</td><td>{[row.length_cm,row.width_cm,row.height_cm].map(v=>v===null?"—":number.format(Number(v))).join(" × ")}</td><td>{row.innpro_match&&<p>FULL: {row.innpro_match.full==="matched"?"Abbinato":row.innpro_match.full==="duplicate"?"EAN duplicato":"EAN mancante"}<br/>Stock LIGHT: {row.innpro_match.light==="matched"?"Verificato":"Da verificare"}</p>}{row.product_info&&<details><summary>Informazioni prodotto</summary>{Object.entries(row.product_info).filter(([,v])=>v).map(([k,v])=><p key={k}><strong>{{brand:"Marca",category:"Categoria",description:"Descrizione",producer_code_standard:"Codice produttore",warranty:"Garanzia"}[k]??k}: </strong>{v}</p>)}</details>}</td>{economicFields.map(f=><td key={f}><input aria-label={`${labels[f]} ${row.sku || row.ean}`} type="number" min="0" max="999999999999" step="any" required disabled={!writable} value={edits[row.id]?.[f] ?? row[f]} onChange={e=>edit(row,f,e.target.value)} /></td>)}</tr>)}</tbody></table></div>
