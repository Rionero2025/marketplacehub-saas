...
export function SellerPublicationPanel({sellerId}:{sellerId:string}) {
  ...
  const pages=Math.max(1,Math.ceil((job?.rows.length??0)/100));
  ...
  async function open(id:string) {
    ...
    setRules({...value.rules});setOptions({});setJob(value);setPage(1);setEdits({});setSelected(value.rows.filter(r=>r.status==="pending").map(r=>r.id));setConfirmation("");setUncertain(false);
  }
  useEffect(()=>{
  ...
  async function preview(){const value=readPublicationJob(await request("/preview","POST",rules),sellerId);if(!value)throw new Error("Anteprima non valida.");setJob(value);setPage(1);setEdits({});setSelected(value.rows.filter(r=>r.status==="pending").map(r=>r.id));setConfirmation("");setUncertain(false);await refresh();}
  ...
  return <div className={styles.panel}>
    ...
    {account&&<section className={styles.card}><h2>2. Regole di pubblicazione</h2><fieldset disabled={!editable}><div className={styles.grid}>
      ...
      </div><div className={styles.actions} role="group" aria-label="Prodotti da pubblicare"><label className={styles.check}><input type="checkbox" checked={rules.selection_mode==="all"} onChange={()=>change({selection_mode:"all"})}/>Seleziona tutti i prodotti</label><label className={styles.check}><input type="checkbox" checked={rules.selection_mode==="range"} onChange={()=>change({selection_mode:"range"})}/>Seleziona da numero prodotto a numero prodotto</label></div><div className={styles.grid}>
      {rules.selection_mode==="range"&&<>{numeric("start","Dal prodotto n. (incluso)",1,1000000,"1")}<label>Al prodotto n. (incluso)<input type="number" min={rules.start} max={rules.start+19999} step="1" value={rules.start+rules.limit-1} onChange={e=>change({limit:Number(e.target.value)-rules.start+1})}/></label></>}
    ...
    <p className={styles.note}>I prezzi vengono ricalcolati dal costo della vista, con spedizione inclusa una sola volta. La commissione serve a stimare il guadagno. I prodotti senza peso restano inclusi. La selezione si applica all’elenco dopo i filtri: tutti i prodotti oppure dal numero iniziale al numero finale, entrambi inclusi. Nell’anteprima puoi correggere i valori prima dell’invio.</p>
    ...
    {job&&<section className={styles.card}><h2>3. Anteprima ed esiti · {labels[job.status]}</h2><div className={styles.summary}><span>Account<strong>{job.account_name}</strong></span><span>Destinazione<strong>{countries[job.rules.storefront]} · {job.rules.playground?"Playground":"REALE"}</strong></span><span>Vista<strong>{job.view_name}</strong></span><span>Intervallo<strong>{job.rules.start}–{job.rules.start+job.total-1} su {job.filtered_total} filtrati</strong></span></div>
      ...
      <p className={styles.note}>La selezione comprende tutte le pagine dell’anteprima. Le righe con errori restano escluse. Modifica le celle e salva prima di pubblicare. Il guadagno viene calcolato da vendita − costo − commissione. Lo SKU composto si aggiorna con EAN, costo e minimo, salvo una tua modifica esplicita dello SKU.</p>
      <div className={styles.table}><table><thead><tr><th>Invia / Riga</th><th>Prodotto</th><th>EAN / SKU</th><th>Quantità</th><th>Peso kg</th><th>Costo totale €</th><th>Vendita €</th><th>Minimo €</th><th>Commissione stimata €</th><th>Guadagno stimato</th><th>Esito</th></tr></thead><tbody>{job.rows.slice((visiblePage-1)*100,visiblePage*100).map(r=><tr key={r.id}><td><input aria-label={`Seleziona riga ${r.position}`} type="checkbox" disabled={!editable||r.status!=="pending"||!["draft","interrupted"].includes(job.status)||uncertain} checked={selectedIds.has(r.id)} onChange={e=>setSelected(e.target.checked?[...selected,r.id]:selected.filter(id=>id!==r.id))}/> {r.position}</td><td>{cell(r,"name","Prodotto")}{r.product_info&&<details><summary>Informazioni FULL</summary>{Object.entries(r.product_info).filter(([,v])=>v).map(([k,v])=><p key={k}><strong>{{brand:"Marca",category:"Categoria",description:"Descrizione",producer_code_standard:"Codice produttore",warranty:"Garanzia"}[k]??k}: </strong>{v}</p>)}<p>Dimensioni cm: {[r.length_cm,r.width_cm,r.height_cm].map(v=>v??"—").join(" × ")}</p></details>}{r.innpro_match&&<small>Stock: {r.innpro_match.light==="matched"?"LIGHT verificato":r.innpro_match.light==="manual"?"modifica manuale":"da verificare"}</small>}</td><td>{cell(r,"ean","EAN")}{cell(r,"sku","SKU")}</td><td>{cell(r,"quantity","Quantità")}</td><td>{cell(r,"weight_kg","Peso kg")}</td>{(["cost","price","minimum_price","commission"] as const).map((key,i)=><td key={key}>{cell(r,key,["Costo totale","Vendita","Minimo","Commissione"][i])}</td>)}<td className={styles.money}>{edits[r.id]?"Da ricalcolare al salvataggio":euro.format(Number(r.profit))}</td><td>{r.problem||labels[r.status]}{r.result_code&&<div>{r.result_code}</div>}</td></tr>)}</tbody></table></div>
      {pages>1&&<div className={styles.actions}><button disabled={visiblePage===1} onClick={()=>setPage(visiblePage-1)}>Pagina precedente</button><span>Pagina {visiblePage} di {pages} · {job.total} prodotti</span><button disabled={visiblePage===pages} onClick={()=>setPage(visiblePage+1)}>Pagina successiva</button></div>}
