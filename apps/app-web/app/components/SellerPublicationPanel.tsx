...
export function SellerPublicationPanel({sellerId}:{sellerId:string}) {
  ...
  const metadata=(key:"shipping_group"|"warehouse"|"logistic_class"|"state_code",label:string,list:string)=><label>{label}<input list={`publication-${list}`} value={rules[key]} onChange={e=>change({[key]:e.target.value})}/><datalist id={`publication-${list}`}>{options[list]?.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</datalist></label>;
