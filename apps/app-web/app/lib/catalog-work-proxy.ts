...
export async function catalogWorkProxy(request: NextRequest, seller: string, parts: string[]) {
  ...
  if (!index && !preview && !save && !resource) return failure(404);
  ...
  try {
    ...
    const parsed = index ? readWorkIndex(raw, seller) : preview || (resource && method === "GET") ? readWorkData(raw, seller, resource ? parts[1] : undefined) : readWorkView(raw);
