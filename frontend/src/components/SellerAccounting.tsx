  ...
  {data&&<section className="panel">
   ...
   <div className="dataTable accountingTable"><table>
    ...
    <tbody>{data.items.map(row=><tr key={row.id}>
     ...
     {writable&&<td><button className="secondaryButton" disabled={busy||Boolean(edit)} onClick={()=>startEdit(row)}>Modifica</button></td>}
    </tr>)}</tbody>
   </table></div>
   {!data.total&&<p>Nessuna riga nel periodo. Verifica i filtri o avvia la sincronizzazione contabile.</p>}
   <div className="settingsActions"><button className="secondaryButton" disabled={busy||Boolean(edit)||offset===0} onClick={()=>setOffset(v=>Math.max(0,v-50))}>Precedente</button><button className="secondaryButton" disabled={busy||Boolean(edit)||offset+50>=data.total} onClick={()=>setOffset(v=>v+50)}>Successiva</button></div>
  </section>}
