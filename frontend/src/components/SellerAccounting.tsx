...
type Data = {items:Row[];total:number;totals:Record<string,number>;missing_rows:number;editable_fields:Record<string,string>};
...
function Panel({sellerId}:{sellerId:number}){
 ...
 const query=new URLSearchParams({account_id:String(account),date_from:from,date_to:to,search,missing_only:String(missing)}).toString();
 ...
 return <>
  ...
  <section className="panel" aria-label="Filtri contabilità"><fieldset disabled={busy||Boolean(edit)} className="accountingFieldset"><div className="settingsForm">
   <label>Account marketplace<select value={account} onChange={e=>{filtersChanged();setAccount(Number(e.target.value))}}><option value="0" disabled>Seleziona account</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.account_name}{a.active?'':' · disattivato'}</option>)}</select></label>
   <label>Dal<input type="date" value={from} onChange={e=>{filtersChanged();setFrom(e.target.value)}}/></label><label>Al<input type="date" value={to} onChange={e=>{filtersChanged();setTo(e.target.value)}}/></label>
  ...
  </div></fieldset><p className="muted">Date in ora italiana. Totali ed Excel includono tutte le righe filtrate, anche oltre la pagina corrente. Massimo 20.000 righe per download.</p></section>
  ...
  {data&&<><div className="stats sellerMetricGrid">{[['Vendite',data.totals.sale],['Acquisti',data.totals.purchase],['Commissioni',data.totals.commission],['Da ricevere',data.totals.payout],['Margine netto noto',data.totals.net_revenue]].map(([label,v])=><section className="panel statCard" key={String(label)}><span>{label}</span><strong>{euro(v)}</strong></section>)}</div><p>{data.total} righe filtrate · {data.missing_rows} con margine da verificare. Gli importi mancanti non sono stimati.</p></>}
