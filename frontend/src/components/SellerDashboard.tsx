...
type Detail = {order_id:string;order_date:string;marketplace:string;status:string;row_key?:string;product_title?:string;ean?:string;composite_sku?:string;supplier?:string;purchase_cost_eur?:number|null;commission_eur?:number;payout_eur?:number|null;extra_cost_eur?:number;sale_eur:number;net_revenue_eur:number|null;partner_share_eur:number|null;our_share_eur:number|null;missing_reason?:string;missing_profit_rows?:number};
...
function Panel({sellerId}:{sellerId:number}){
 ...
 function exportPage(){
   if(!data)return;const columns=['order_id','order_date','marketplace','product_title','ean','sale_eur','purchase_cost_eur','commission_eur','payout_eur','extra_cost_eur','net_revenue_eur','partner_share_eur','our_share_eur','missing_reason'] as const;
 ...
 return <>
  ...
  <section ref={detailRef} className="panel" style={{marginTop:20}}><h2>Dettaglio dei valori</h2><div className="settingsForm"><label>Vista<select value={view} onChange={e=>{setView(e.target.value);setOffset(0)}}><option value="lines">Righe contabili</option><option value="orders">Ordini unici</option><option value="missing">Righe da verificare</option></select></label><label>Cerca ordine o prodotto<input value={search} onChange={e=>{setSearch(e.target.value);setOffset(0)}} placeholder="Ordine, EAN, SKU o fornitore"/></label></div>
   ...
   <div className="tableWrap"><table className="dataTable"><thead><tr><th>Ordine / Data</th><th>{view==='orders'?'Stato':'Prodotto'}</th><th>Marketplace</th><th>Vendite</th>{view!=='orders'&&<><th>Acquisto</th><th>Commissioni</th><th>Da ricevere</th><th>Costi extra</th></>}<th>Margine</th><th>Quota Seller</th><th>Quota gestore</th><th>Verifica</th></tr></thead><tbody>{data?.details.items.map((r,i)=><tr key={`${r.row_key||r.order_id}:${i}`}><td>{r.order_id}<small className="sellerSubline">{dayLabel(r.order_date)}</small></td><td>{r.product_title||r.status}<small className="sellerSubline">{r.ean||r.composite_sku}</small></td><td>{r.marketplace}</td><td>{euro(r.sale_eur)}</td>{view!=='orders'&&<><td>{euro(r.purchase_cost_eur)}</td><td>{euro(r.commission_eur)}</td><td>{euro(r.payout_eur)}</td><td>{euro(r.extra_cost_eur)}</td></>}<td>{euro(r.net_revenue_eur)}</td><td>{euro(r.partner_share_eur)}</td><td>{euro(r.our_share_eur)}</td><td>{r.missing_reason||(r.missing_profit_rows?`${r.missing_profit_rows} righe`:'—')}</td></tr>)}</tbody></table></div>
