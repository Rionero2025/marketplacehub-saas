...
from services.object_storage import atomic_write, object_store, sha256_bytes, storage_config
...
def read_bytes(
    *, local_path: str | Path | None = None, storage_key: str = "", expected_sha256: str = "",
    ...
    if path is not None and path.is_file():
        ...
        if not expected or sha256_bytes(payload).lower() == expected:
            return payload
        if not str(storage_key or "").strip():
...
def materialize(
    *, namespace: str, identity: str, filename: str, storage_key: str,
    ...
    if preferred is not None and preferred.is_file():
        cached = preferred.read_bytes()
        if not expected_sha256 or sha256_bytes(cached) == expected_sha256.strip().lower():
            return preferred
    payload = read_bytes(storage_key=storage_key, expected_sha256=expected_sha256)
    target = (CACHE_DIR / safe_filename(namespace.replace("/", "_"), "files")
              / safe_filename(identity, "item") / sha256_bytes(payload) / safe_filename(filename))
